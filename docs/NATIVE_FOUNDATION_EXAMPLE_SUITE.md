...
| Map route and annotation | run map graph/current coordinate; annotation is presentation-only | exact map point or drawing-input stop | room transition proves next domain | T0 and owner T1 pass; native decision provider/T2/T3 pending |
...
| Reward/CardReward/Map | reward set -> card-reward options -> run map | exact reward/holder/proceed/map controls | owner changes are explicit; no immediate-observe causal claim | owner discriminator T1 pass; final cross-domain T3 pending |
| Event/Rest/Treasure | each current room model/choice owner | typed source-local controls | domain task/room transition | T0 route audit; migration and final runtime pending |
