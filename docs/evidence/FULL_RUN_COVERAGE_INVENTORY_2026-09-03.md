...
| new game / character selection | `NCharacterSelectScreen` and its native `StartRunLobby`/embark controls | typed `character_select` surface with exact character/ascension/embark bindings | no current Human gameplay ingress; selection is run-setup provenance, not a gameplay root | lifecycle-provenance; live-unqualified | exact start metadata and Human-origin capture remain outside the current recording profile |
| run configuration / seed / mode | native run setup state and Host seed provenance | Host controls expose only the explicitly configured standard-run contract | no Annotator action authority | lifecycle-provenance / intentionally bounded | custom modes and arbitrary setup fields are outside this profile; no UI inference is promoted |
